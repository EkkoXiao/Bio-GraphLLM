...
from autogllight.nas.space.graces_space.op_graph_classification import *
...
from autogllight.nas.space.graces_space.pooling_zoo import filter_features, filter_perm
...
class PoolingMixedOp(nn.Module):
    ...
    def forward(self, x, edge_index, edge_weights, data, batch, mask, weights):
        ...
        x, edge_index, edge_weight, batch, perm = filter_perm(sum(new_x), edge_index, sum(new_edge_weight), batch, sum(new_perm), th=-1e-6)
...
class Network(nn.Module):
  ...
  def __init__(self, in_dim, hidden_size, num_layers=3, dropout=0.5, epsilon=0.0, args=None, with_conv_linear=False, mol=False, virtual=False):
    ...
    for i in range(num_layers):
        self.pooling_layers.append(PoolingMixedOp(hidden_size, self.pooling_ratio[i]))

    self.pooling_global = PoolingMixedOp(hidden_size * (num_layers + 1), args.pooling_ratio)
  ...
  def forward(self, data, mode='none', graph_alpha = None):
    ...
    # pooling 之后经过 layer7 生成整个的 graph embedding
    ...
    for i in range(batch_size):
        nodes = (batch == i).nonzero(as_tuple=True)[0]  
        node_embs.append(torch.cat([x[nodes], torch.zeros(max_nodes - len(nodes), x.size(1), device=x.device)]))
        node_mask[i, :len(nodes)] = 1 
    node_embs = torch.stack(node_embs, dim=0)

    return node_embs, node_mask
  ...
  def _initialize_alphas(self):
    ...
    if self.args.model_type == 'darts':
        self.log_na_alphas = Variable(1e-3*torch.randn(self.num_layers, num_na_ops), requires_grad=True)
        self.log_act_alphas = Variable(1e-3*torch.randn(self.num_layers, num_act_ops), requires_grad=True)
        #self.log_pool_alphas = Variable(1e-3*torch.randn(self.num_layers, num_pool_ops).cuda(), requires_grad=True)
        self.log_pool_alphas = Variable(1e-3*torch.randn(1, num_pool_ops), requires_grad=True)
        self.log_readout_alphas = Variable(1e-3*torch.randn(self.num_layers+1, num_readout_ops), requires_grad=True)
        self.log_la_alphas = Variable(1e-3*torch.randn(1, num_la_ops), requires_grad=True)
    else:
        self.log_na_alphas = Variable(
            torch.ones(self.num_layers, num_na_ops).normal_(self._loc_mean, self._loc_std), requires_grad=True)
        self.log_act_alphas = Variable(
            torch.ones(self.num_layers, num_act_ops).normal_(self._loc_mean, self._loc_std), requires_grad=True)
        ...
        self.log_pool_alphas = Variable(
            torch.ones(1, num_pool_ops).normal_(self._loc_mean, self._loc_std), requires_grad=True)
        self.log_readout_alphas = Variable(
            torch.ones(self.num_layers + 1, num_readout_ops).normal_(self._loc_mean, self._loc_std),
            requires_grad=True)

        self.log_la_alphas = Variable(torch.ones(1, num_la_ops).normal_(self._loc_mean, self._loc_std),
                                      requires_grad=True)
